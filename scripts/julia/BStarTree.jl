function BSTree(ARGS)
    ...
    for i in qntd
        ...
        for j = 1:10
            ...
            if (ARGS[1] == "-sh") 
                run(`./main 4 $i 1 $randNum key$j`)
                run(`./main 4 $i 2 $randNum key$j`)
                run(`./main 4 $i 3 $randNum key$j`)
            elseif (ARGS[1] == "-bat")
                run(`main.exe 4 $i 1 $randNum key$j`)
                run(`main.exe 4 $i 2 $randNum key$j`)
                run(`main.exe 4 $i 3 $randNum key$j`)
            else
